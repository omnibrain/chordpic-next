...
const useUpdatePasswordMutation = () =>
  ...
  // reads it off the client's own session, so an unauthenticated call simply
  // comes back with an error instead of needing a guard here.
  useMutation((newPassword: string) =>
    createClient().auth.updateUser({ password: newPassword }),
  );
